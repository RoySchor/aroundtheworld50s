...
class TipsManager:
    ...
    def convert_html_to_markdown(self, html_content):
        """Convert HTML content back to markdown format for editing"""
        if not html_content:
            return ""
        ...
        lists = re.finditer(list_pattern, content, re.DOTALL)
        for list_match in lists:
            ...
            # Join with newlines and add an extra newline after the list
            markdown_list = "\n".join(markdown_items) + "\n"
            content = content.replace(list_match.group(0), markdown_list)
    ...
    def load_tips_file(self, file_path):
        """Load and convert tips content from JSON file for editing"""
        try:
