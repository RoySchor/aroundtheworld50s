...
export const metadata: Metadata = {
  title: "Destinations",
  description: "Explore all the destinations we've visited around the world on an interactive map.",
  openGraph: {
    type: "website",
    siteName: SITE_NAME,
    locale: "en_US",
...
export default async function DestinationsPage() {
  const [posts, countriesWithCovers] = await Promise.all([
    getPublishedPosts(),
    getAllCountriesWithCovers(),
  ]);
  ...
  const overrideMap = new Map(
    countriesWithCovers
      .filter((c): c is CountryWithCover & { overrideCover: string } => !!c.overrideCover)
  ...
  for (const post of posts) {
    const locationKey =
      post.state && post.countryCode === "US" ? `USA-${post.state}` : post.country;
    if (seen.has(locationKey)) continue;
    seen.add(locationKey);
    mapLocations.push({
      locationKey,
      displayName: post.state ? `${post.state}, ${post.country}` : post.country,
      countrySlug: post.countrySlug,
      countryCode: post.countryCode,
      backgroundImage: overrideMap.get(post.countrySlug) ?? post.backgroundImage,
    });
  }
  const sortedLocations = mapLocations.sort((a, b) => a.displayName.localeCompare(b.displayName));

  const heroUrl = buildCloudinaryUrl(STATIC_ASSETS.destinationsPageBg, "w_1920");
