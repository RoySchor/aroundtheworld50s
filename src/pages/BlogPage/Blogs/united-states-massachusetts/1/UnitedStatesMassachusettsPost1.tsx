...
import { UNITED_STATES_MASSACHUSETTS_POST_1 } from "./UnitedStatesMassachusettsPost1.constants.ts";
import { ContentSection } from "./UnitedStatesMassachusettsPost1.types";
import { getImagePathFromBlogPost } from "../../BlogPost.utils.ts";

const UnitedStatesMassachusettsPost1 = () => {
  const PlymouthMA = (
    <MapEmbed
      title="Plymouth_MA"
  ...
  const renderContent = (section: ContentSection) => {
    switch (section.layout.type) {
      ...
      case "itinerary-with-map":
        itinerary = UNITED_STATES_MASSACHUSETTS_POST_1.itineraries[section.layout.mapIndex];
        return (
          <TwoColumnLayout
            leftPane={{
              type: "list",
      ...
      case "image-grid":
        return <ImageGrid
          ...
          blogPath="united-states-massachusetts/1"
        />;
      case "two-column":
        return (
          <TwoColumnLayout
            leftPane={{
              ...
              imageUrl:
                section.layout.leftType === "image"
                  ? getImagePathFromBlogPost(UNITED_STATES_MASSACHUSETTS_POST_1, section.leftImage || "")
                  : undefined,
            ...
            rightPane={{
              ...
              imageUrl:
                section.layout.rightType === "image"
                  ? getImagePathFromBlogPost(UNITED_STATES_MASSACHUSETTS_POST_1, section.rightImage || "")
                  : undefined,
              imageAlt: section.layout.imageAlt,
              content:
  ...
  return (
    <div className="page-container">
      <div
        ...
        style={{
          backgroundImage: `url(${getImagePathFromBlogPost(UNITED_STATES_MASSACHUSETTS_POST_1, UNITED_STATES_MASSACHUSETTS_POST_1.backgroundImage || "")})`,
        }}
      >
        <div className="fixed-background-text-container">
          <div className="fixed-background-title fixed-background-no-margin">
            {UNITED_STATES_MASSACHUSETTS_POST_1.header}
          </div>
        </div>
      </div>
      ...
      <div className="container">
        <div className="page-content">
          <div className="post-title">{UNITED_STATES_MASSACHUSETTS_POST_1.title}</div>

          <div className="post-subtitle">{UNITED_STATES_MASSACHUSETTS_POST_1.subtitle}</div>

          <div
            className="post-description"
            dangerouslySetInnerHTML={{ __html: UNITED_STATES_MASSACHUSETTS_POST_1.description }}
          />

          {UNITED_STATES_MASSACHUSETTS_POST_1.tipsSection && (
            <div className="post-bolded-text post-tips-section-container">
              {UNITED_STATES_MASSACHUSETTS_POST_1.tipsLink ? (
                <a
                  href={UNITED_STATES_MASSACHUSETTS_POST_1.tipsLink}
                  className="post-tips-link"
                  dangerouslySetInnerHTML={{ __html: UNITED_STATES_MASSACHUSETTS_POST_1.tipsSection }}
                />
              ) : (
                <div dangerouslySetInnerHTML={{ __html: UNITED_STATES_MASSACHUSETTS_POST_1.tipsSection }} />
              )}
            </div>
          )}

          {UNITED_STATES_MASSACHUSETTS_POST_1.content.map((item, index) => (
...
export default UnitedStatesMassachusettsPost1;
